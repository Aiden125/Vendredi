	...
	<div id="content">
		...
		<div class="center">
			<c:if test="${paging.startPage > paging.blockSize }">
				[ <a href="${conPath }/qna/replyList.do?pageNum=${paging.startPage-1}" class="dis_inline">이전</a> ]
			</c:if>
			<c:forEach var="i" begin="${paging.startPage }" end="${paging.endPage }">
				...
				<c:if test="${i != paging.currentPage }">
					[ <a href="${conPath }/qna/replyList.do?pageNum=${i}" class="dis_inline">${i }</a> ]
				</c:if>
			</c:forEach>
			<c:if test="${paging.endPage < paging.pageCnt }">
				[ <a href="${conPath }/qna/replyList.do?pageNum=${paging.endPage+1}" class="dis_inline">다음</a> ]
			</c:if>
		</div>
	</div>
